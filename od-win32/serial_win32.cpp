...
#define SERIALDEBUG 3 /* 0, 1, 2 3 */ // AXEL-DIAG: was 0
...
static void checkreceive_serial (void)
{
	...
	if (!canreceive()) {
		// AXEL-DIAG: log blocked receive, throttled
		static int axel_blocklog;
		if ((axel_blocklog++ & 1023) == 0) {
			write_log(_T("SERIAL-DIAG: canreceive blocked (data_in_serdatr=%d safe=%d)\n"), data_in_serdatr, safe_receive ? 1 : 0);
		}
		return;
	}
